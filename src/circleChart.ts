 ...
 *
 * @interface
 * @property {CircleChartDataPoint[]} dataPoints - Set of data points the visual will render.
 * @property {number} dataMax                 - Maximum data value in the set of data points.
 */
interface CircleChartViewModel {
    dataPoints: CircleChartDataPoint[];
    dataMax: number;
    settings: CircleChartSettings;
}

/**
 * Interface for CircleChart data points.
 *
 * @interface
...
/**
 * Interface for CircleChart settings.
 *
 * @interface
 ...
 */
function visualTransform(options: VisualUpdateOptions, host: IVisualHost): CircleChartViewModel {
    let dataViews = options.dataViews;
    let defaultSettings: CircleChartSettings = {
        enableAxis: {
            show: false,
            fill: "#000000",
        },
    ...
    let viewModel: CircleChartViewModel = {
        dataPoints: [],
        dataMax: 0,
        settings: <CircleChartSettings>{}
    };
    ...
    let dataValue = categorical.values[0].source.roles.measure ? categorical.values[0] : null;
    let detail = categorical.values[0].source.roles.detail ? categorical.values[0] : null;
    if(categorical.values[1]) {
        dataValue = categorical.values[1].source.roles.measure ? categorical.values[1] : dataValue;
        detail = categorical.values[1].source.roles.detail ? categorical.values[1] : detail;
    }
    if(!dataValue) {
        return viewModel;
    }
    ...
    let circleChartDataPoints: CircleChartDataPoint[] = [];
    let dataMax: number;
    ...
    let circleChartSettings: CircleChartSettings = {
        enableAxis: {
    ...
    for (let i = 0, len = Math.max(category.values.length, dataValue.values.length); i < len; i++) {
        ...
        circleChartDataPoints.push({
            ...
            detail: detail == null ? "" : `${detail.values[i]}`
        });
    }
    ...
    return {
        dataPoints: circleChartDataPoints.sort((a, b) => <number>b.value - <number>a.value),
        dataMax: dataMax,
        settings: circleChartSettings,
    };
}
...
export class CircleChart implements IVisual {
    private svg: Selection<any>;
    private host: IVisualHost;
    private selectionManager: ISelectionManager;
    private barContainer: Selection<SVGElement>;
    private xAxis: Selection<SVGElement>;
    private barDataPoints: CircleChartDataPoint[];
    private circleChartSettings: CircleChartSettings;
    ...
    /**
     * Creates instance of CircleChart. This method is only called once.
     *
     * @constructor
    ...
    public update(options: VisualUpdateOptions) {
        let viewModel: CircleChartViewModel = visualTransform(options, this.host);
        let settings = this.circleChartSettings = viewModel.settings;
        ...
        let radius: number = Math.min(width, height) / 2.7;
        ...
        if (settings.enableAxis.show) {
            let margins = CircleChart.Config.margins;
        ...
        this.xAxis
            .style("font-size", Math.min(height, width) * CircleChart.Config.xAxisFontMultiplier)
        ...
        this.xAxis.selectAll("circle").remove();
        if(settings.enableAxis.show) {
            viewModel.dataPoints.map(d => {
                let categoryCircle = this.xAxis
                    .append("circle")
                    .classed('circle', true);
                categoryCircle
                    .style("fill", d.color)
        ...
        CircleChart.wordBreak(textNodes, xScale.bandwidth(), height);
        textNodes.attr("fill", settings.enableAxis.fill);
        textNodes.each(function() { 
            d3.select(this).attr("fill", settings.enableAxis.fill);
        });
        var ticks = this.xAxis.selectAll(".tick");
        // ticks.selectAll("circle").remove();
        // ticks.each(function() { 
        ...
        barSelectionMerged.classed('circle', true);

        const opacity: number = viewModel.settings.generalView.opacity / 100;

        barSelectionMerged
            .style("fill", (dataPoint: CircleChartDataPoint) => dataPoint.color)
            .style("stroke", (dataPoint: CircleChartDataPoint) => dataPoint.strokeColor)
            .style("stroke-width", (dataPoint: CircleChartDataPoint) => `${dataPoint.strokeWidth}px`)
            .style("fill-opacity", opacity)
            .style("stroke-opacity", opacity)
            .attr("r", d => (height - yScale(<number>d.value)) / height * radius)
            .attr("cx", width / 2)
            .attr("cy", height / 2);
        ...
        this.tooltipServiceWrapper.addTooltip(this.barContainer.selectAll('.circle'),
            (tooltipEvent: TooltipEventArgs<CircleChartDataPoint>) => this.getTooltipData(tooltipEvent.data),
            (tooltipEvent: TooltipEventArgs<CircleChartDataPoint>) => tooltipEvent.data.selectionId
        );
        ...
        var pathData = arcGenerator({
            ...
            innerRadius: (height - yScale(<number>this.calculateAverage())) / height * radius - 4,
            outerRadius: (height - yScale(<number>this.calculateAverage())) / height * radius + 4
        });
        this.barContainer.selectAll("path").remove();
        this.barContainer.selectAll(".textValue").remove();
        if(settings.averageLine.show) {
            let chosenColor = this.getColorValue(settings.averageLine.fill);
        ...
        viewModel.dataPoints.sort((a, b) => (<number>a.value - <number>b.value)).map(d => {
            ...
            if(d.value == this.getMin()) {
                ...
                let dx = textValue
                    ...
                    .attr("x", x + "px")
                    .attr("y", y + "px")
                    .attr("fill", previous_color)
                    .attr("text-anchor", "left")
                    .style("font-size", fontSizeValue + "px")
                    .node().getComputedTextLength();
                if(x + dx > width) {
                    textValue.attr("x", width - dx);
                }
                previous_color = d.color;
            }
        });
    ...
    private syncSelectionState(
        selection: Selection<CircleChartDataPoint>,
        ...
        if (!selectionIds.length) {
            const opacity: number = this.circleChartSettings.generalView.opacity / 100;
            selection
                .style("fill-opacity", opacity)
                .style("stroke-opacity", opacity);
        ...
        selection.each(function (barDataPoint: CircleChartDataPoint) {
            const isSelected: boolean = self.isSelectionIdInArray(selectionIds, barDataPoint.selectionId);

            const opacity: number = isSelected
                ? CircleChart.Config.solidOpacity
                : CircleChart.Config.transparentOpacity;
    ...
    public enumerateObjectInstances(options: EnumerateVisualObjectInstancesOptions): VisualObjectInstanceEnumeration {
        ...
        if (!this.circleChartSettings ||
            !this.circleChartSettings.enableAxis ||
            !this.barDataPoints) {
        ...
        switch (objectName) {
            case 'enableAxis':
                objectEnumeration.push({
                    ...
                    properties: {
                        show: this.circleChartSettings.enableAxis.show,
                        fill: this.circleChartSettings.enableAxis.fill,
                    },
                    selector: null
                });
                break;
            ...
            case 'generalView':
                objectEnumeration.push({
                    objectName: objectName,
                    properties: {
                        opacity: this.circleChartSettings.generalView.opacity,
                        showHelpLink: this.circleChartSettings.generalView.showHelpLink
                    },
                    validValues: {
                        opacity: {
                            numberRange: {
            ...
            case 'averageLine':
                objectEnumeration.push({
                    objectName: objectName,
                    properties: {
                        show: this.circleChartSettings.averageLine.show,
                        displayName: this.circleChartSettings.averageLine.displayName,
                        fill: this.circleChartSettings.averageLine.fill,
                        showDataLabel: this.circleChartSettings.averageLine.showDataLabel
                    },
                    selector: null
                });
                break;
    ...
    private handleAverageLineUpdate(height: number, width: number, yScale: ScaleLinear<number, number>) {
        ...
        let fontSize = Math.min(height, width) * CircleChart.Config.xAxisFontMultiplier;
        let chosenColor = this.getColorValue(this.circleChartSettings.averageLine.fill);
        ...
        this.averageLine
            ...
            .style("display", (this.circleChartSettings.averageLine.show) ? "initial" : "none")
        ...
        this.averageLine.select("#averageLineLabel")
            ...
            .style("fill", this.circleChartSettings.averageLine.showDataLabel ? chosenColor : "none");
    }
    ...
    private calculateAverage(): number {
        ...
        this.barDataPoints.forEach((value: CircleChartDataPoint) => {
